...
def compile_source_to_custom_bytecode(source_code):
    """
    Custom VM Compiler: Chuyển đổi mã nguồn Lua chữ thuần thành các đoạn
    Opcode nhị phân ẩn danh cấu trúc cao nhằm loại bỏ loadstring.
    """
    tokens = re.findall(r'(\d+)|([a-zA-Z_][a-zA-Z0-9_]*)|("[^"]*"|\'[^\']*\')|([\+\-\*\/\=\>\<\~\,\.\(\)\{\}\[\]\;])|(\s+)', source_code)
    bytecode_bytes = bytearray()
    
    # Header Byte định danh cấu trúc VM riêng biệt
    bytecode_bytes.append(0x8B) 
    
    for t in tokens:
        num, var, string_lit, op, space = t
        if num:
            # Opcode 1: Khối Hằng số Số (Numeric Constant)
            bytecode_bytes.append(1)
            val = int(num) & 0xFFFF
            bytecode_bytes.append((val >> 8) & 0xFF)
            bytecode_bytes.append(val & 0xFF)
        elif var:
            # Opcode 2: Khối Tên biến / Từ khóa (Identifier)
            bytecode_bytes.append(2)
            encoded_var = var.encode('utf-8', errors='ignore')
            bytecode_bytes.append(len(encoded_var) & 0xFF)
            bytecode_bytes.extend(encoded_var)
        elif string_lit:
            # Opcode 3: Khối Hằng số Chuỗi (String Literal)
            bytecode_bytes.append(3)
            clean_str = string_lit[1:-1].encode('utf-8', errors='ignore')
            bytecode_bytes.append((len(clean_str) >> 8) & 0xFF)
            bytecode_bytes.append(len(clean_str) & 0xFF)
            bytecode_bytes.extend(clean_str)
        elif op:
            # Opcode 4: Khối Toán tử / Ký tự điều hướng (Operator)
            bytecode_bytes.append(4)
            encoded_op = op.encode('utf-8', errors='ignore')
            bytecode_bytes.append(len(encoded_op) & 0xFF)
            bytecode_bytes.extend(encoded_op)
        elif space:
            # Opcode 5: Dấu cách tối giản hóa luồng code
...
def ironbrew_total_wrapped_v12_1(source_code):
    ...
    # BƯỚC 1: Biên dịch mã nguồn sang định dạng Opcode nhị phân của VM
    custom_bytecode = compile_source_to_custom_bytecode(source_code)
    
    # BƯỚC 2: Bọc ma trận xoay XOR + Hex lên chuỗi Opcode nhị phân đó
    ...
    # Khởi tạo tên biến ngẫu nhiên bảo vệ Trình thông dịch VM
    ...
    # Các biến xử lý nghiệp vụ Vòng lặp VM (Virtual Machine Execution Engine)
    ...
    # GIỮ NGUYÊN 5000 DÒNG MÃ RÁC SIÊU NẶNG QUA MÁY NGHIỀN
    junk_pieces = []
    for _ in range(5000):
        v_junk = random_var()
    ...
    # LÕI TRÌNH THÔNG DỊCH ẢO (VIRTUAL MACHINE INTERPRETER CORES) - KHÔNG LOADSTRING
    bit_and_interpreter_core = (
        ...
        f"local function {v_decrypt_func}({random_var()}) "
        f"local {v_tab}={{}}; "
        ...
        f"{v_vm_pc} = {v_vm_pc} + 1; "
        f"if {v_vm_op} == 1 then "
        f"local {v_vm_data} = {v_buffer}[{v_vm_pc}]*256 + {v_buffer}[{v_vm_pc}+1]; "
        f"{v_vm_pc} = {v_vm_pc} + 2; "
        f"{v_vm_res}[#{v_vm_res}+1] = tostring({v_vm_data}); "
        f"激=nil; "
        ...
        f"local {random_var()}, {random_var()} = nil, nil; "
